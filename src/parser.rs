...
impl Parser {
    pub fn new(mut source: Lexer) -> Parser {
        let buffer = source.get_token();
        Parser{source, buffer}
    }

    fn get_token(&mut self) -> Token {
        replace(&mut self.buffer, self.source.get_token())
    }
    ...
    fn parse_binary_expr(&mut self, mut lhs: Node, precedence: i8) -> Node {
        while let Token::Operator(op) = &self.look_ahead() {
            if precedence > op.precedence() {
                ...
                if let Token::Operator(op) = self.get_token() {
                    ...
                    while is_binary_op(&self.buffer) {
                        if let Token::Operator(ahead) = &self.buffer {
                            ...
                            if let Token::Operator(ahead) = self.get_token() {
                    ...
                    lhs = Node::Binary(BinaryNode {op, lhs: Box::new(lhs), rhs: Box::new(rhs)})
                }
    ...
    fn parse_prototypes(&mut self) -> Node {
        if let Token::Identifier(name) = self.get_token() {
            if let Token::LeftParenthesis = self.get_token() {
                let mut args = Vec::new();
                loop {
                    match self.get_token() {
                        Token::Comma => continue,
    ...
    fn parse_top_level_expr(&mut self) -> Node {
        let body = Box::new(self.parse_expr());
        Node::Function(FunctionNode {prototype: PrototypeNode {name: Box::new([]), args: Vec::new()}, body})
    }
